...
def plot_profile(file, vmin, colorbar_times, cmap = "jet"):
    ...
    # sv_fig("../Output/" + file.split("/")[2].split("_")[2] + "mask")
    ...
    data_name = file.replace(".fits", "_data.dat").replace("../Fits/", "../Data/")
    
    # np.savetxt(data_name, data_all)
    # print(f"We have saved the {data_name}")
...
# file_chandra_smooth = '../Fits/G310_i_Chandra_r250_smooth.fits'
file_chandra_smooth = '../Fits_4-subband-useful/hard_flux.img.smooth-py.fits'
...
r_list, data_profile_askap, data_err_askap, data_max_askap = plot_profile(file_askap, vmin = RMS_ASKAP, colorbar_times = 6 ,cmap = 'CMRmap_r')
...
# Chandra 的流量还要再确定一下
r_list, data_profile_chandra_smooth, data_err_chandra_smooth, data_max_chandra_sm = plot_profile(file_chandra_smooth, vmin = RMS_Chandra_sm, colorbar_times = 12, cmap = "ocean_r") 
#%%
...
def plot_profile_2D(ax):
    ...
    ax.errorbar(r_list, data_profile_askap, data_err_askap, fmt='o',ecolor='red',color='red',elinewidth=2,capsize=4, label = "ASKAP", alpha = 0.5)
...
plot_profile_2D(ax)
plt.legend(loc='lower right',bbox_to_anchor=[1,1.02],
            ncol = 3, shadow = False, fancybox= True) #loc="upper right",, 
beamsize = [60,72]
position = [0.1, 0.1]
ax.plot(beamsize, position, color='green', linewidth = 3)
ax.text(80, 0.08, f"beam = ~12\"")
rsize = [60,66]
position = [0.2, 0.2]
ax.plot(rsize, position, color='green', linewidth = 3)
ax.text(80, 0.18, f"$\Delta$r = {120/20}\"")


ax.set_xlim(-2, 125)
ax_in = fig.add_axes([0.4, 0.4, 0.5, 0.38])
plot_profile_2D(ax_in)
# ax_in.set_xlim(43, 125)
ax_in.set_xlim(30, 125)
ax_in.set_ylim(0, 0.04)
...
# %%
dataFile_askap = "../Data/G310_i_EMU_r250_data.dat"
dataFile_chandra = "../Data/G310_i_Chandra_r250_data.dat"
dataFile_chandra_sm = "../Data/G310_i_Chandra_r250_smooth_data.dat"
data_askap = np.loadtxt(dataFile_askap)[-11:-1,0]
data_chandra = np.loadtxt(dataFile_chandra)[-11:-1,0]
data_chandra_sm = np.loadtxt(dataFile_chandra_sm)[-11:-1,0]
print("ASKAP", (120/20) * (10 + np.argmax(data_askap)))
print("Chandra", (120/20)* (10 + np.argmax(data_chandra)))
print("(120/20)* (10 + np.argmax(data_chandra_sm)))
# %%
